...
# UNTESTED
      ...
      MessageParser.new(:found_feature, /^Found a (?<feature>[ \w]+)./),
      MessageParser.new(:found_shop, /^Found (?<shop>.+)./),
      MessageParser.new(:shaft, /^You fall through a shaft( for (?<floors>\d) floors)?!/),
      MessageParser.new(:passed_gate, /^You pass through the gate./),
      MessageParser.new(:got_item, /^Got (?<item>[ \w]+)/),
      MessageParser.new(:bought_item, /^Bought (?<item>[ \d\w\{\}\+\-\',\(\)]+) for (?<gold>\d+) gold pieces/),
      MessageParser.new(:identified_item, /^Identified (?<item>[ \d\w\{\}\+\-\']+)( \(You found it on level (?<level>\d{1,2}) of (?<branch>[ \w]+)\))?/),
      MessageParser.new(:banished_monster, /^Banished ([ \w]+)/),
      ...
      MessageParser.new(:exited, /^Escaped (?<branch>.+)/),
