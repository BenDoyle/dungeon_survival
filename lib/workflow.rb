...
class Workflow
  ...
  def parse_message(message)
    message_parsers = [
      ...
      MessageParser.new(:transmutation, /^([Aa]n?)? (?<from>[ \-\w]+) changed into (an?)? (?<to>[ \-\w+])/),
      ...
      MessageParser.new(:noticed_monster, /.+ was a mimic/, monster: 'mimic'),
      MessageParser.new(:draconian_matures, /^Your scales.+/),
      MessageParser.new(:gained_ally, /^Gained (?<monster>[ \w]+) as an ally/),
      MessageParser.new(:lost_ally, /^Your ally (?<monster>[ \w]+)? died/),
      MessageParser.new(:lost_ally, /^(?<monster>[ \w]+) \(ally\) was/),
      ...
      MessageParser.new(:runed_door, /^[Rr]uned door/),
      ...
      MessageParser.new(:shuffle_card, /^Shuffle card\:.+/),
      ...
      MessageParser.new(:noticed_monster, /([Aa]n?) (?<monster>[ \w]+)/),
